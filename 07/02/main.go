...
func main() {
	...
	freeSpaceNeeded := 30000000 - (70000000 - calculateDirSize(rootDir))
	candidates := findCandidatesToDelete(rootDir, freeSpaceNeeded)
	dirToDelete := findDirToDelete(candidates)

	fmt.Println(dirToDelete)

}

func findDirToDelete(candidates []int) int {
	var min int
...
func findCandidatesToDelete(currentDir *directory, targetSize int) []int {
	// find how much free space we need
	candidates := []int{}
	currSize := calculateDirSize(currentDir)
	if currSize >= targetSize {
		candidates = append(candidates, currSize)
	}
	for _, child := range currentDir.children {
		candidates = append(candidates, findCandidatesToDelete(child, targetSize)...)
	}
	return candidates
}
